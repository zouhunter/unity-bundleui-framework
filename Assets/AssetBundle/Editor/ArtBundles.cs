...
public class ArtBundles : EditorWindow
{
    ...
    private void DrawCtrlButtons()
    {
        ...
        {
            if (GUILayout.Button("加载文件", GUILayout.Width(80), GUILayout.Height(45)))
            {
                OnLoadButtonClicked();
            }
            if (GUILayout.Button("批量生成", GUILayout.Width(80), GUILayout.Height(45)))
            {
    ...
    void OnLoadButtonClicked()
    {
        ...
        {
            case ArtType.Texture:
                FileUtility.Recursive(artAssetsFolder, "jpg", deepSet, action: (x) => { allFiles.Add(x); });
                FileUtility.Recursive(artAssetsFolder, "png", deepSet, action: (x) => { allFiles.Add(x); });
                break;
            case ArtType.Model:
                FileUtility.Recursive(artAssetsFolder, "fbx", deepSet, action: (x) => { allFiles.Add(x); });
                break;
            case ArtType.Prefab:
                FileUtility.Recursive(artAssetsFolder, "prefab", deepSet, action: (x) => { allFiles.Add(x); });
                break;
            case ArtType.Material:
                FileUtility.Recursive(artAssetsFolder, "mat", deepSet, action: (x) => { allFiles.Add(x); });
                break;
            default:
                break;
        }
